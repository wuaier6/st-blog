select
        ...
        GROUP BY dept_id ) A
        left join (select dept_id, count(DISTINCT user_id) as man_count from z_htn_user_screen_info  where sex=0 GROUP BY dept_id) B on A.dept_id=B.dept_id
        left join (select dept_id, count(DISTINCT user_id) as woman_count from z_htn_user_screen_info where sex=1  GROUP BY dept_id) C on A.dept_id=C.dept_id
        left join (select dept_id, count(DISTINCT user_id) as agemin65_count from z_htn_user_screen_info where age<65   GROUP BY dept_id) D on A.dept_id=D.dept_id
        left join (select dept_id, count(DISTINCT user_id) as agemin75_count from z_htn_user_screen_info where age>=65 and age<75 GROUP BY dept_id) E on A.dept_id=E.dept_id
        left join (select dept_id, count(DISTINCT user_id) as agemax75_count from z_htn_user_screen_info where age>=75   GROUP BY dept_id) K on A.dept_id=K.dept_id
        left join (select zhusi.dept_id, count(DISTINCT zhusi.user_id) as blood_pressure_type_one_count from (select * from z_htn_user_screen_info where id in(
            ...
            where zhrr.blood_pressure_type=1   GROUP BY zhusi.dept_id
        ...
        left join (select zhusi.dept_id, count(DISTINCT zhusi.user_id) as blood_pressure_type_two_count from (select * from z_htn_user_screen_info where id in(
            ...
            GROUP BY dept_id,user_id) ) as zhusi
            inner join z_htn_risk_report zhrr on zhusi.id=zhrr.htn_screen_id
            where zhrr.blood_pressure_type=2   GROUP BY zhusi.dept_id
        ...
        left join (select zhusi.dept_id, count(DISTINCT zhusi.user_id) as blood_pressure_type_three_count from (select * from z_htn_user_screen_info where id in(
            select max(id) id  from z_htn_user_screen_info           GROUP BY dept_id,user_id)  ) zhusi
            inner join z_htn_risk_report zhrr on zhusi.id=zhrr.htn_screen_id
            where zhrr.blood_pressure_type=3   GROUP BY zhusi.dept_id
        ...
        left join (select zhusi.dept_id, count(DISTINCT zhusi.user_id) as coronary_heart_disease_count from (select * from z_htn_user_screen_info where id in(
            ...
            GROUP BY dept_id,user_id)) zhusi where zhusi.coronary_heart_disease_flg=1
             GROUP BY zhusi.dept_id) G on A.dept_id=G.dept_id
        left join (select zhusi.dept_id, count(DISTINCT zhusi.user_id) as intake_salt_count from (select * from z_htn_user_screen_info where id in(
            ...
            GROUP BY dept_id,user_id) ) zhusi where zhusi.intake_salt_flg=1
             GROUP BY dept_id) H on A.dept_id=H.dept_id
        left join (select zhusi.dept_id, count(DISTINCT zhusi.user_id) as hypotensor_count from (select * from z_htn_user_screen_info where id in(
           ...
           GROUP BY zhusi.dept_id) I on A.dept_id=I.dept_id
